## What changed in 3.8.15

Added the `repository` and `license` fields to `plugin.json`. Both are "highly recommended" by Anthropic's plugin-directory guidelines, so the automated review sees them explicitly. Metadata only — no behavior change.

## What changed in 3.8.12

Replaced the plugin body with the 3.8.7 build (2026-08-10), which is newer work than the
3.8.11 that was on this branch despite the lower version number — the branch's number had
climbed one bump per review fix while its content sat at 08-06. That build adds the
customer logo header (`Drata_getCompany`, `.cust` / `.custname`), the deterministic HTML
filename convention, and framework auto-selection.

Re-applied on top, because the 3.8.7 build predates them and a straight overwrite would
have reopened findings two reviewers already signed off:

- **`test_source` bound to `ACORN`** in `drata-monitoring-report`. Unbound, the mandatory
  `Production = raw − Code` subtraction returns `raw − 0 = raw` — the pooled figure the
  section exists to prevent — while all three pre-ship identities still pass, because they
  hold trivially when every Code count is zero.
- **`drata-personnel-report` and `drata-control-identify-gaps` command descriptions.** Both
  promised what their skills forbid (a named roster; root-cause analysis). The description
  is what an agent routes on before the SKILL.md loads.
- **`drata-help` catalog line and `drata-framework-report` handoff** aligned with the same
  contract; readiness comparison repointed at `drata-control-report`, which produces it.
- **`bucket` means only the four categories**; individual records are "evidence item".
  Against a 333-item library the old wording rendered "Library: 333 buckets" to a customer.
- **`write-safety`**: batch partial-failure contract restored, and `personnel` dropped from
  the governed-writes header — no personnel-write skill ships.
- **`drata-brand-kit`**: base-plus-extension model restored, merged with the new `.cust`
  classes. The 3.8.7 wording ("one identical full copy") would have had a restyle drop
  nine skills' load-bearing extension rules.
