...
compatibility: "Requires Drata MCP with tools: Drata_getCompany, Drata_searchControls, Drata_listRequirements, Drata_listPolicies, Drata_createControl, Drata_updateControl; optional for the evidence and monitor fixes: Drata_listEvidence, Drata_createEvidence, Drata_updateEvidence, Drata_searchMonitoringTests (absent → context-carrying handoff to drata-evidence-resolve-gaps). Requires create:control / update:control scope + a permitting role."
metadata:
...
already named a specific change ("put Jane Doe on DCF-15"), skip the menu and run the fix directly:
...
- "Put Jane Doe on DCF-15." *(direct fix — no menu)*
