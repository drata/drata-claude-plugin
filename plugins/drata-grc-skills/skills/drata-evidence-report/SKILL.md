...
compatibility: "Requires Drata MCP with tools: Drata_getCompany, Drata_listEvidence; optional Drata_searchMonitoringTests (automated-failing cut)"
---
...
`Drata_listEvidence(statuses=[...])` is **partly reliable** — each was checked against the payload on a test tenant. Use it exactly this way:
...
- `Drata_searchMonitoringTests(check_result_status="FAILED", size=50)` — optional, only for the automated-evidence-failing cut (match a `TEST_RESULT` version to its test by `source` name). **Page it to completion:** this is a name join, not a count, so a failing test left on page 2 silently makes its `TEST_RESULT` evidence look healthy. An unmatched `TEST_RESULT` item is reported as unmatched, never assumed current.
...
3. **Bucket every item from the sweep payload — all four states, including Expired and Expiring
   soon.** The precedence rule (Needs artifact wins when there is no current version at all) can only
   be applied to items you can *see*, and the two `statuses=` calls return a scalar `totalCount` with
   no rows. You cannot subtract the artifact-less items from a bare number, so bucketing from those
   counts double-counts every item that is both artifact-less and expired, understates Valid, and
   makes the four-bucket sum unachievable. The sweep already carries what you need —
   `renewalSchema.renewalDate` for both renewal states and `versions[]` for the artifact state — so
   derive all four client-side from the one payload and label them *Calculated*.
   **The two server counts are a cross-check, never the source.** Compare them against your
   pre-precedence Expired and Expiring-soon tallies: they should be close, and they will usually sit
   slightly *above* your post-precedence buckets, because items that lost the precedence tie-break
   moved to Needs artifact. A large divergence means the sweep did not page to completion — fix that
   before rendering rather than reconciling by adjusting a bucket.
...
| `EXPIRING_SOON` window | Not documented in the API — give the observed renewal-date range from the swept `renewalDate` values (the `size=1` status call returns no rows and so no range), never a fixed "30 days" |
