...
compatibility: "Requires Drata MCP with tools: Drata_listEvidence, Drata_searchControls, Drata_searchMonitoringTests"
---
...
`Drata_listEvidence(statuses=[…])` is **partly reliable**. Each was checked against the payload in a
live tenant, so use them exactly this way:

| Status | Verdict | Evidence |
|---|---|---|
| `EXPIRED` | **Use it** | 53 items, every sampled `renewalDate` in the past — coherent |
| `EXPIRING_SOON` | **Use it** | 13 items, every `renewalDate` 2–19 days out. **Drata's look-ahead window is not documented in the API and the sample had nothing past 19 days, so it is not confirmed to be 30.** Never print a day threshold you have not measured — say "Renewal soon" and give the observed range |
| `NEEDS_SOURCE` | **Do not use** | 107 items, including one with a current `URL` version — it returns items that plainly have a source |
| `NEEDS_ARTIFACT` | **Cross-check, never quote alone** | returned 0; confirm against `versions[]` before reporting a zero |
| `READY` | **Do not use for arithmetic** | 265 + the non-READY statuses (173) = 438 against a 333-item library, so the statuses do not partition |
...
| `versions[]` → `current` | which version counts; no current entry = the bucket is empty |
...
| `description`, `implementationGuidance` | empty = nobody can tell what belongs in the bucket |
...
4. **Match automated evidence to its test by name.** TEST_RESULT versions carry the test name in `source` (e.g. evidence "GCP VPC Subnet Segmentation" ← version source "GCP VPC Subnet Segmentation"). Match on that. **Say so in the artifact** — it is a name match, not an ID join, so an unmatched item is reported as unmatched rather than assumed healthy.
...
### Needs artifact — [a] buckets
By framework. An item feeding two frameworks appears under both.

| Framework | Buckets | Controls fed | Next step |
|---|---:|---:|---|
| SOC 2 | 12 | 19 | Attach the artifact in the Drata UI (no MCP upload), or link a URL |
*Framework counts overlap and do not sum to [a] — an item can serve several frameworks.*

### Automated evidence failing — [b] buckets
Backed by a monitoring test whose last result is FAILED — the bucket is not stale, its source is broken.

| Evidence | Test (source name) | Controls fed | Next step |
|---|---|---:|---|
| GCP VPC Subnet Segmentation | GCP VPC Subnet Segmentation | 1 | Fix the integration → drata-monitoring-report |
*Matched on version source name, not a test ID. [n] TEST_RESULT buckets could not be matched to a test — listed, not assumed healthy.*

### Renewal — [c] overdue · [d] renewal soon
| Evidence | Renewal date | Days | Owner | Next step |
|---|---|---:|---|---|
| [name] | 2026-06-01 | 59 overdue | [owner / —] | Re-file and re-date → drata-evidence-resolve-gaps |

### Callout — buckets mapped to no control
[n] buckets feed no control, so they count toward no audit and no framework. Either map them
...
empty-bucket case is not repeated here; it is the Needs artifact bucket.

**Scope the guidance check to manual buckets only.** A `TEST_RESULT`-backed bucket is refreshed by
its monitoring test, not by a person, so implementation guidance is not expected and its absence is
not a finding — counting automated buckets here inflates the number with items nobody should act on.
Exclude them, and say the count is manual-only. The other three checks apply to every bucket.
> **No owner** — [n] buckets · nobody accountable for refreshing them
> **No renewal schedule** (`NONE`) — [n] buckets · will never prompt again, so they go stale silently
> **No description** — [n] buckets · an auditor cannot tell what the bucket is meant to prove
> **No implementation guidance** — [n] **manual** buckets · the next person collecting it has no instructions

### Now · Next · Watch
**Now** — automated failing buckets under a not-ready control: the source is broken, not the paperwork. → drata-monitoring-report
**Next** — needs-artifact buckets in the framework you are auditing against. → drata-evidence-resolve-gaps
