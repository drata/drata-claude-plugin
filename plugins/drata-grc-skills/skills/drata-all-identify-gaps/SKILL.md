...
## Key tools
- `Drata_searchControls` — not-ready controls (`is_ready=false`). For work already in flight use the per-control `flags.hasTicket` boolean from `expand=["flags"]`; **never the `ticket_status` filter, which is broken — see step 4.**
- `Drata_searchMonitoringTests` — failed checks (`check_result_status="FAILED"`).
- `Drata_searchRisks` — open risks by inherent score (a number, never a severity name Drata does not store), and treatments past `anticipated_completion_date`.
...
- `Drata_listWorkspaces` — scope; one workspace → use it, several → ask which once.

## Workflow
1. **Resolve scope.** `Drata_listWorkspaces()`. One workspace → use it silently. If the user named one, use it. **Several and they named none → ask which one, once, and reuse it for the rest of the session.** There is no "primary" or "default" workspace in Drata — no `isPrimary` or `isDefault` field exists — so picking one yourself is inventing a scope the user never chose and silently narrowing every count in the briefing. Then pass it as `workspace_id` on every workspace-scoped call. Vendors, policies and personnel are account-scoped — never ask which workspace for them.
2. **Gather current failures in parallel** (structured filters, never semantic `query`, so counts are exact):
   ...
   **2b. "Personnel failing 1+ check" needs its own call — a facet cannot produce it.** Facet buckets
   are *per-check* counts, and with `size=1` there are no rows, so nothing can reduce them to a count
   of distinct people: summing them double-counts everyone failing two checks, and taking the largest
   bucket understates the total. Get the distinct-person figure from the roster —
   `Drata_listPersonnel(compliance_status=<failing>, employment_status=[…the same list…],
   include_total_count=True, size=1)` — and read its `totalCount`. Keep the per-check facets for the
   *breakdown* only, and say plainly that they overlap and will not sum to the headline. **If that
   roster count cannot be obtained, omit the headline rather than assembling one from the facets.**
...
3. **Gather commitments by date — server-side filters, same parallel batch.** Split into **overdue** (already past a committed date) and **due soon** (a committed date approaching but not yet passed), and keep them in separate groups — due-soon items are NOT overdue. Do not compute a combined "overdue commitments" total or headline number:
   ```
   Drata_searchRisks(anticipated_completion_date_lt="<today, YYYY-MM-DD>",
     status=["ACTIVE"], size=25)        # treatments past their committed date
                                        # _lt, not _lte: a treatment committed for TODAY is not
                                        # yet past its date, and this row is strictly overdue.
                                        # If only _lte is accepted, pass yesterday's date.
   ...
   # No ticket_status filter — it is not status-aware in this MCP: the numeric enum lands on a boolean
   # has_ticket column, so IN_PROGRESS is INVERTED (it returns the controls with NO ticket) and
   # ARCHIVED returns has-any-ticket. Ticket state = per-control flags.hasTicket, from the step-2 expand=["flags"] fetch.
   ```
   Union the two vendor calls and dedupe by vendor ID before counting vendors once.
4. **Surface not-ready controls with no ticket yet — the actionable net-new work.** The only trustworthy ticket signal is the per-control `flags.hasTicket` boolean (from `expand=["flags"]`); **the `ticket_status` filter is broken in this MCP and must never be used for anything: it is not status-aware — the numeric enum (`IN_PROGRESS`=0, `ARCHIVED`=1) is applied to a boolean `has_ticket` column, so `IN_PROGRESS` is *inverted* and returns the controls with **no** ticket, while `ARCHIVED` returns the has-any-ticket set. It is an upstream API defect, not something a filter combination can work around, and its results look plausible enough to publish — so never derive a count, a ranking, or an "in flight" figure from it.** Page the `is_ready=false` set with `expand=["flags"]`, `size=50`, following the cursor **until pagination is exhausted** (several pages on a typical estate — a partial scan is not acceptable), and count `hasTicket=false` = the exact number of not-ready controls with **no remediation ticket yet** (call it **M**). Show **M as a single exact count** — never a ratio, never the word "sampled," never a "(sampled)" suffix. **If M = 0 (every not-ready control already has a ticket — 100% coverage), omit the row entirely: there is no gap to flag.** Only if the not-ready set exceeds 300 controls may you stop early — then show `≥M (first 300 scanned)`. When ranking Now/Next, deprioritise controls whose `flags.hasTicket=true` (already ticketed) in favour of the un-ticketed ones — never re-recommend ticketed work.
...
| Multiple workspaces | Ask which one, once, then reuse it — never pick a "primary", which is not a Drata concept; offer the multi-workspace roll-up (→ drata-executive-report) |
