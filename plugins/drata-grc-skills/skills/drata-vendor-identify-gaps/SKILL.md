...
permission: "write (optional: create risk on explicit confirmation)"
compatibility: "Requires Drata MCP with tools: Drata_listVendors, Drata_getVendor, Drata_listVendorSecurityReviews, Drata_listVendorDocuments, Drata_searchRisks. Escalation also needs Drata_createRisk + create:risk scope."
---
...
3. **Rank (Calculated), then name the work.** Rank by `next_review_deadline` bucket (OVERDUE
   ahead of DUE_SOON), then days overdue descending, then residual `risk` level, then
   `impact_level`; show those fields beside the rank. Every row gets a **named next action** — refreshed SOC 2, chase the questionnaire,
...
**`category: "NONE"` and `category: null` both mean unassigned, and you must treat them the same.**
This is the one trap in the set: `risk: "NONE"` is a real residual level, so a rule that reads
`"NONE"` as a value on one field and as absence on the other will quietly mis-sort vendors. Business
unit unset appears as either, depending on how the record was created.
...
forever and inflate the overdue figure several-fold (`OVERDUE` returns 62 total against 14 current
on a real account). Never publish the raw total.
...
|---|---|---|---|---|
| Acme | <span class="lvl"><i style="background:#D53641"></i>High</span> | <span class="lvl"><i style="background:#D53641"></i>Critical</span> | <span class="flag">Review 47d overdue</span> | owner@ |

**Five columns, and this is the shape:**
- **Vendor name only — never the numeric id**, and never `Acme (4821)`. The id is plumbing; it means
...
- **One `.flag` chip per row** naming the failing check, with the ageing folded in where it exists
  (`Review 47d overdue`, `Never reviewed`, `No owner`). Additional flags go in the same cell as
...
**Reviewed:** 2 security reviews · SOC 2 Type II (ended [date]) · 1 open risk (Tool Calls)
**Gaps:** No penetration test · questionnaire 14 months old (Gap)
...
- "Drata, run due diligence on Acme and give me an approve/reject recommendation."
- "Should we renew Datadog? Pull their reviews, documents, and open risks."
- "Drata, log the missing-pen-test finding on Acme as a risk."
