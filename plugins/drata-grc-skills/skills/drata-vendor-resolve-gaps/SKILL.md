...
description: >
  ...
  archives on request. Use for 'fix my vendors', 'assign vendor owners', 'record the Acme
  decision'. Write access.
area: Third-Party & Vendor Risk
permission: write (create / update / delete vendor)
compatibility: "Requires Drata MCP with tools: Drata_listVendors, Drata_getVendor, Drata_createVendor, Drata_updateVendor, Drata_deleteVendor; Drata_listVendorSecurityReviews optional (review-history context on request). Requires create:vendor / update:vendor / delete:vendor scope + a permitting role."
---
...
change ("archive the old Sentry record"), skip the menu and run the fix directly: read → preview →
confirm → write.
