...
description: >
  ...
  in-scope framework, then reports framework readiness % with per-area pass rates,
...
compatibility: "Requires Drata MCP with tools: Drata_listWorkspaces, Drata_listRequirements, Drata_searchControls, Drata_searchMonitoringTests, Drata_listEvidence, Drata_searchRisks"
---
...
- **After the workspace is chosen → sweep it for in-scope frameworks and ask which one**, listing
  what you found with requirement counts. Wait for the answer.
...
- **Exactly one workspace → skip the first question silently** and go straight to the framework question.
...
denominator the user never chose. **Passing `workspace_id` yourself, or letting the MCP's own
...
1. **The deliverable is the branded artifact, not a chat answer — in every mode, including
   ...
   with the `.drata` theme, the KPI tile and the requirement grid. **Never reply
...
2. **Never start until workspace and framework are both settled.** Step 1 is a blocking gate, not a
   preamble. Do not pull requirements, controls, evidence or anything else "to get going" while a
   question is outstanding.
3. **One framework per report.** If the user has not chosen one, ask — do not substitute an
...
6. **The KPI row is ONE tile: `[ready] of [in-scope]` with `· [N]%` on the label line.** Not four,
   ...
   is itself inferred, and in the run that prompted this rule it read `3 of 4` while the report
   showed two areas, neither of them complete. If a figure is not `ready of in-scope`, it is not a
   KPI tile.
...
anything else, and make no other tool call until both answers are in.
...
that workspace for its in-scope frameworks and **offer those as the second question** — never ask
the user to name a framework from memory, and never offer one that is not in scope there:
```
...
workspace. **Present the frameworks you found, with their requirement counts, and let the user
pick.** Skipping this question and reporting on every framework at once is the failure this step
exists to prevent. If the user already named both workspace and framework, ask nothing.
...
Probe scope:
`Drata_listRequirements(framework_tag=["<X>"], is_in_scope=true, size=500)`. Rows →
**IN-SCOPE MODE**. None → re-run with `is_in_scope=false`.
...
- **Zero rows in both states → STOP. Produce no report.** The framework is not loaded in this
  workspace at all, so there is nothing to measure against. Say that in one line, list the
...
ready-vs-not-ready comparison and the ownership caveat are **drata-control-identify-gaps's** deliverable —
...
fill at `ready ÷ total`, labelled **`6 of 9 ready · 67%`** — count, total, the word *ready*, then the
percentage. Never `6/9`, never a bare percentage with no bar, never a number without its
...
**No overall readiness bar.** The KPI tile already states `4 of 51 · 8%`; a full-width bar directly
beneath it repeats that and nothing else. The grid is the first visual.

**1 · The requirement grid, ungrouped.** One 15px cell per requirement, 3px gaps, ready cells
`#2E4DFF` first, then `#D9DCDE`, each with a `title` carrying its code and state. **The grid carries
its own readiness figure** — a key beneath it reading `Ready 4 · 8%` and `Not ready 47 · 92%`, so
the grid stands on its own if it is the only thing a reader looks at. This is the honest default: it
shows every requirement individually and assumes nothing about structure.
...
**Above ~150 requirements** the grid stops being scannable: keep the KPI tile, and replace the
grid with per-area bars **only if** the grouping test above passes. If it does not, show the top
areas by gap as a plain table instead — never a grid nobody can read.

Markup — grid with its own key:
```html
<div class="grid">
  <i class="cell on" title="CC1.1 — ready"></i><i class="cell" title="CC1.2 — not ready"></i>…
</div>
<div class="key">
  <span><span class="sw" style="background:#2E4DFF"></span>Ready <b>4</b> · 8%</span>
  <span><span class="sw" style="background:#D9DCDE"></span>Not ready <b>47</b> · 92%</span>
</div>
```
Per-area row (secondary section only) — **never inside a `<table>`**:
```html
<div class="area">
  <div class="arow"><span class="nm">Availability</span><span><b>2</b> of 4 ready · 50%</span></div>
...
what produced `36 of 40` colliding with the bar: the cell has no room to give, so the text and the
...
would name — not the raw code prefix.** `category` is null on every requirement I have pulled, so
...
**Sourcing.** `category` is null on every requirement across every framework I have pulled — SOC 2,
HIPAA, GDPR, FedRAMP 20x — so **the API supplies no grouping and never will resolve this for
